...
class SystemSubState extends MusicBeatSubstate
{
	var textMenuItems:Array<String> = ['Cache All sound and song', 'Exit'];
	...
	public function new()
	{
		...
		var versionShit:FlxText = new FlxText(5, FlxG.height - 18, 0, "System State | Press Enter to enable setting", 12);
		...
		// 		txt.alpha = 0.6;
		// });
		textOptions = new FlxText(0, FlxG.height * 0.9 + 0, FlxG.width, "Cache All Sound and Song on the assets folder\nWhen open game", 35);
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			...
			{
				case "Cache All sound and song":
					if(!save.data.options.contains("Cache All sound and song")){
						save.data.options.push("Cache All sound and song");
					}else{
						save.data.options.remove("Cache All sound and song");
					}
					trace("Cache All sound and song change");
				case "Exit":
					FlxG.state.closeSubState();
					FlxG.state.openSubState(new OptionsSubState());
			}
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch (textMenuItems[curSelected]){
            case "Cache All sound and song":
                textOptions.text = "Cache All Sound and Song on the assets folder\nWhen open game";
			case "Exit":
