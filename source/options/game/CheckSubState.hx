...
{
	var textMenuItems:Array<String> = ['Skip Check Version', 'Exit'];
	...
	public function new()
	{
		...
		var versionShit:FlxText = new FlxText(5, FlxG.height - 18, 0, "Check State" + " | " + "Press Enter to enable setting", 12);
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			...
			{
				case "Skip Check Version":
					if(!save.data.options.contains("Skip Check Version")){
						save.data.options.push("Skip Check Version");
					}else{
						save.data.options.remove("Skip Check Version");
					}
					trace("Skip Check Version change");
				case "Exit":
					FlxG.state.closeSubState();
					FlxG.state.openSubState(new OptionsSubState());
			}
