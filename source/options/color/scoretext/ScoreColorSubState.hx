package options.color.scoretext;
...
{
	var textMenuItems:Array<String> = ['RED', 'YELLOW', 'GREEN', 'BLUE', 'LIME', 'Exit'];
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			...
			{
				case "RED":
					if(!save.data.options.contains("RED")){
						save.data.options.push("RED");
					}else{
						save.data.options.remove("RED");
					}
					save.data.options.remove("YELLOW");
					save.data.options.remove("GREEN");
					save.data.options.remove("BLUE");
					save.data.options.remove("LIME");
					trace("Red change");
				case "Exit":
					FlxG.state.closeSubState();
					FlxG.state.openSubState(new OptionsSubState());
